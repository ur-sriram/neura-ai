...
## 48. FREE APIs AND OPEN-SOURCE RESOURCES CATALOGUE `[PROPOSED]`

This section catalogues every free API and open-source GitHub repository identified as directly
useful to NE-Setu. Each entry is mapped to the pipeline stage it serves, includes access method,
usage limits, and a concrete integration note. All entries marked **VERIFIED** have been confirmed
reachable at time of writing.

> [!NOTE]
> The project already uses snapshot-at-build-time for external APIs (Section 0.2, Conflict 4).
> Every live API below should be called once during the build window, validated, and baked into
> the database. The adapters are written behind an interface so live vs. snapshot is a config switch.

---

### 48.1 Weather & Rainfall APIs

#### Open-Meteo — Hourly Rainfall + Forecasts `[VERIFIED]`

| Field | Detail |
|---|---|
| **URL** | `https://api.open-meteo.com/v1/forecast` |
| **Auth** | None — no API key required |
| **Free limit** | 10,000 calls/day for non-commercial use |
| **Data** | Current conditions, hourly & daily forecasts; `precipitation`, `rain`, `weather_code`, `wind_speed_10m`, `visibility` |
| **Historical archive** | `https://archive-api.open-meteo.com/v1/archive` — free, goes back to 1940 |
| **Pipeline stage** | SENSE (Stage 1) — primary weather feed; PREDICT (Stage 3) — antecedent rainfall input |
| **Integration note** | Fetch hourly precipitation for the Guwahati–Shillong bbox at build time. Replay historical monsoon archive (2019–2023) to seed the simulation timeline. Example: `?latitude=25.5&longitude=91.9&hourly=precipitation,rain&past_days=92` |

```python
# Snapshot fetch — run once at build time
...
#### IMD API Platform — Official Indian Met Dept `[ASSUMED — requires registration]`

| Field | Detail |
|---|---|
| **URL** | `https://api.imd.gov.in/public/api_reference.html` |
| **Auth** | Free registration; IP whitelisting may apply |
| **Data** | District-wise warnings, AWS/ARG station nowcasts, 7-day forecasts, subdivision rainfall |
| **Pipeline stage** | SENSE (Stage 1) — corroborating signal for antecedent rainfall; future production integration |
| **Integration note** | Use as a secondary trust-weighted source in the Event Ingestor (Section 16). In the prototype, Open-Meteo is the primary; IMD is registered and stubbed as the `LiveWeatherProvider` implementation. |
| **Fallback** | Open-Meteo covers the same data with higher reliability for prototyping |

---

#### SACHET / NDMA — National Disaster Alert Portal `[VERIFIED]`

| Field | Detail |
|---|---|
| **URL** | `https://sachet.ndma.gov.in` |
| **Auth** | None — CAP (Common Alerting Protocol) RSS feed is public |
| **Data** | Official government disaster alerts; flood, cyclone, landslide warnings by district |
| **Pipeline stage** | SENSE / UNDERSTAND — high-trust event source (government-issued → trust weight 0.90) |
| **Integration note** | Parse the CAP-XML RSS feed at build time to seed the `Event` table with real historical alerts for the Meghalaya/Assam corridor. In production, poll every 15 min. |

---

### 48.2 Elevation & Terrain APIs

#### Open-Meteo Elevation — Instant, No Key `[VERIFIED]`

| Field | Detail |
|---|---|
| **URL** | `https://api.open-meteo.com/v1/elevation?latitude=25.5&longitude=91.9` |
| **Auth** | None |
| **Free limit** | Same 10,000/day as forecast endpoint |
| **Pipeline stage** | ASSESS (Stage 4) — slope derivation for Accessibility Score |
| **Integration note** | Batch-fetch elevation for all OSM road segment midpoints. Compute grade from consecutive segment elevations (`Δelev / length`). Cheap and sufficient at MVP fidelity. |

---

#### OpenTopography Global DEM API — SRTM 30m `[VERIFIED — free tier]`

| Field | Detail |
|---|---|
| **URL** | `https://portal.opentopography.org/API/globaldem` |
| **Auth** | Free account registration; API key emailed immediately |
| **Free limit** | 50 calls/24h (non-academic) / 200 calls/24h (academic) — adequate for one bounding-box download |
| **Resolution** | SRTM GL1 (30 m); Copernicus GLO-30 (30 m) also available |
| **Pipeline stage** | Data preparation — download the bbox DEM raster once at build time, derive slope grid locally with `rasterio`/`GDAL` |
| **Integration note** | One API call downloads the full Guwahati–Jowai bbox (~100×150 km) as a GeoTIFF. Compute slope locally and join to OSM edge GeoDataFrame. Preferred over per-point Open-Meteo elevation calls for slope accuracy. |

---

#### ISRO Bhuvan — CartoDEM 10m (India-specific) `[ASSUMED — registration required]`

| Field | Detail |
|---|---|
| **URL** | `https://bhuvan.nrsc.gov.in` |
| **Auth** | Free government registration (Indian institution preferred) |
| **Data** | CartoDEM — 10 m resolution national DEM; higher accuracy than SRTM in the Khasi hills |
| **Pipeline stage** | Data preparation — premium elevation layer if registration completes within the build window |
| **Integration note** | Use as a SHOULD item: if SRTM suffices for demo fidelity, Bhuvan is deferred. If grade accuracy on 9%+ slopes matters for the demo, CartoDEM is worth the registration time. |

---

### 48.3 Road Network & Mapping APIs

#### Overpass API — OpenStreetMap Queries `[VERIFIED]`

| Field | Detail |
|---|---|
| **URL** | `https://overpass-api.de/api/interpreter` |
| **Auth** | None |
| **Free limit** | Community-use rate limiting; large single extracts recommended over repeated queries |
| **Pipeline stage** | Data preparation — extract road graph with bridge `maxweight`, lane restrictions, surface tags |
| **Integration note** | Use `OSMnx` (which wraps Overpass) for the graph build. For additional bridge data, fire a targeted Overpass QL query: `way["bridge"]["maxweight"](bbox)` to get weight-tagged bridges specifically in the corridor. |

```python
# Targeted bridge weight query
import requests
query = """
[out:json][timeout:60];
way["bridge"]["maxweight"](25.3,91.3,26.3,92.6);
out geom;
"""
resp = requests.post("https://overpass-api.de/api/interpreter", data=query)
bridges = resp.json()["elements"]
```

---

#### Nominatim Geocoding — No Key Required `[VERIFIED]`

| Field | Detail |
|---|---|
| **URL** | `https://nominatim.openstreetmap.org/search` |
| **Auth** | None; 1 request/sec fair-use policy |
| **Data** | Forward/reverse geocoding; place search |
| **Pipeline stage** | Data preparation — geocode the 30–40 depot/village/facility locations to lat/lon |
| **Integration note** | Batch geocode all locations at build time, cache results. Do not call in the demo runtime path. |

---

#### Geofabrik OSM Extracts — Direct Download `[VERIFIED]`

| Field | Detail |
|---|---|
| **URL** | `https://download.geofabrik.de/asia/india.html` |
| **Auth** | None — direct `.osm.pbf` download |
| **Data** | Full OSM data for India as a PBF file; or smaller sub-region extracts |
| **Pipeline stage** | Data preparation — alternative to Overpass for the full road graph; more reliable for production |
| **Integration note** | Download the Northeast India extract (`india-northeast-latest.osm.pbf`) and feed to `OSMnx` with `ox.graph_from_file()`. Recommended if Overpass rate limits are hit during the build. |

---

### 48.4 Optimization & Solver Libraries (Open Source)

#### Google OR-Tools / CP-SAT `[VERIFIED — Apache 2.0]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/google/or-tools` |
| **Install** | `pip install ortools` |
| **Pipeline stage** | OPTIMIZE (Stage 5) — joint cargo→vehicle→route CP-SAT assignment (Section 12.4) |
| **Integration note** | Already specified as the primary solver. CP-SAT handles ~40×12 assignment in milliseconds. The repo includes VRP examples directly usable as templates for Section 12.4's decision variables. |

---

#### VROOM — High-Performance VRP Engine `[VERIFIED — BSD 2-Clause]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/VROOM-Project/vroom` |
| **Integration** | REST API (self-hosted Docker) or Python via `vroom-python` |
| **Pipeline stage** | OPTIMIZE (Stage 5) — optional fast-path for multi-vehicle routing pre-solve before CP-SAT |
| **Integration note** | SHOULD item. Use VROOM to generate an initial warm-start assignment, then pass to CP-SAT for hard-constraint refinement. Reduces solver wall-time on larger instances. |

---

### 48.5 Geospatial Libraries (Open Source)

#### OSMnx — Road Network Download & Analysis `[VERIFIED — MIT]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/gboeing/osmnx` |
| **Install** | `pip install osmnx` |
| **Pipeline stage** | Data preparation + OPTIMIZE (Stage 5) — graph construction, grade calculation, NetworkX integration |
| **Integration note** | Core library. Use `ox.graph_from_bbox()` for the corridor. Attach elevation via `ox.elevation.add_node_elevations_google()` or via the Open-Meteo elevation endpoint. Compute edge grades with `ox.elevation.add_edge_grades()`. |

---

#### H3-py — Uber Hexagonal Indexing `[VERIFIED — Apache 2.0]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/uber/h3-py` |
| **Install** | `pip install h3` |
| **Pipeline stage** | ASSESS (Stage 4) — regional hex aggregation for accessibility heatmap (Section 13.3) |
| **Integration note** | Use resolution 7 (avg hex area ~5 km²) for regional heatmap, resolution 8 (~1.2 km²) for local cluster warnings. `h3.latlng_to_cell(lat, lng, 7)` maps each road segment midpoint to a hex. |

---

#### GeoPandas + Shapely `[VERIFIED — BSD]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/geopandas/geopandas` |
| **Install** | `pip install geopandas` |
| **Pipeline stage** | Throughout — GeoDataFrame operations for spatial joins, flood zone intersections, segment filtering |
| **Integration note** | Use `geopandas.sjoin()` to intersect road edges with hazard polygons (flood inundation footprints). Central to the flood probability model (Section 11.2). |

---

#### PyLandslide — ML Landslide Susceptibility `[VERIFIED — MIT]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/WRHGroup/PyLandslide` |
| **Install** | `pip install pylandslide` |
| **Pipeline stage** | PREDICT (Stage 3) — reference implementation for landslide susceptibility mapping (Section 11.1) |
| **Integration note** | Study the susceptibility index methodology rather than using directly. Our T1+T2 layered approach (Section 11.1) replicates the core idea with terrain features native to our OSM+DEM build. Use PyLandslide's calibration patterns as a cross-reference for weight selection. |

---

#### Landslide Susceptibility Framework — iamtekson `[VERIFIED — MIT]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/iamtekson/landslide_susceptibility` |
| **Pipeline stage** | PREDICT (Stage 3) — workflow reference for Section 11.1 training corpus generation |
| **Integration note** | Provides a complete end-to-end workflow: DEM → slope/aspect/curvature features → spatial join with inventory → model training → susceptibility map. Adapt for our synthetic corpus approach: use the feature pipeline, substitute historical inventory with our simulation-generated closures. |

---

### 48.6 India Flood & Disaster Data

#### India Flood Inventory — IIT Delhi / HydroSense `[VERIFIED — research dataset]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/HydroSense-IITD/India-Flood-Inventory` |
| **Data** | Geospatial historical flood event data for India; research-grade |
| **Pipeline stage** | Data preparation — historical inundation footprint for the Brahmaputra basin used as the flood probability model's spatial prior (Section 11.2) |
| **Integration note** | Download the flood polygons for Assam/Meghalaya. Join to the road graph with `geopandas.sjoin()` to assign `historical_inundation_class` to each edge. This replaces the `[ASSUMPTION]`-flagged Sentinel-1 dependency in Section 11.2 with a verifiable open dataset. |

---

#### data.gov.in — Open Government Data Platform `[VERIFIED]`

| Field | Detail |
|---|---|
| **URL** | `https://data.gov.in` |
| **Auth** | Free API key after registration |
| **Data** | Thousands of government datasets: road lengths by district, health facility locations, population census, commodity movement statistics |
| **Pipeline stage** | Data preparation — seed the 30–40 location table; population weights for equity term (Section 12.6) |
| **Integration note** | Search for: "health facilities Meghalaya", "village population Assam district", "national highways northeast India". CSV downloads are available without API for most datasets. |

---

### 48.7 ETA & Traffic Prediction

#### LibCity — Unified Urban Spatio-Temporal Prediction `[VERIFIED — Apache 2.0]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/LibCity/Bigscity-LibCity` |
| **Pipeline stage** | PREDICT (Stage 3) — reference for ETA prediction model architecture (Section 11.3) |
| **Integration note** | Do not use LibCity directly (too heavy for 27-hour build). Use as a reference for LightGBM ETA residual model features and evaluation methodology. Their travel-time datasets demonstrate the feature set that matters: segment length, road class, gradient, time-of-day, weather. |

---

#### gboeing/travel-time-prediction `[VERIFIED — MIT]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/gboeing/travel-time-prediction` |
| **Pipeline stage** | PREDICT (Stage 3) — concrete implementation reference for OSMnx + ML ETA workflow |
| **Integration note** | Direct inspiration for Section 11.3's approach. Uses OSMnx road features + ML to predict travel time residuals. Adapt the feature extraction pipeline to add our weather and gradient penalty columns, then train LightGBM on the synthetic corpus. |

---

### 48.8 Visualisation & Frontend

#### MapLibre GL JS — Open Map Renderer `[VERIFIED — BSD 3-Clause]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/maplibre/maplibre-gl-js` |
| **Install** | `npm install maplibre-gl` |
| **Pipeline stage** | Frontend — live map digital twin (F19); accessibility heatmap (F04) |
| **Integration note** | Already specified in File 3 `[SOURCE]`. Use with a free basemap tile source (OpenStreetMap via `https://tile.openstreetmap.org/{z}/{x}/{y}.png` or Stadia Maps free tier). Colour road segments by LNS state using `map.setPaintProperty()` on a GeoJSON layer. |

---

#### deck.gl — WebGL Geospatial Layers `[VERIFIED — MIT]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/visgl/deck.gl` |
| **Install** | `npm install deck.gl` |
| **Pipeline stage** | Frontend — H3 hex heatmap layer; vehicle animation overlay |
| **Integration note** | `H3HexagonLayer` renders the hex accessibility heatmap in one declarative component. `ScatterplotLayer` animates vehicle positions on the simulation clock. Already mentioned in File 3 `[SOURCE]`. |

---

#### OpenTripPlanner — Multimodal Routing Reference `[VERIFIED — LGPL]`

| Field | Detail |
|---|---|
| **GitHub** | `https://github.com/opentripplanner/OpenTripPlanner` |
| **Pipeline stage** | FUTURE reference — multimodal ferry/rail routing (Section 9, F34) |
| **Integration note** | Do not integrate in the MVP. The OSMnx+NetworkX custom graph (Section 0.2, Conflict 5) is the MVP routing engine. OTP is the recommended path for the Future Scope multimodal expansion. Record here so the architecture team knows the seam. |

---

### 48.9 Integration Priority Matrix

The table below orders all resources by build-criticality, so the 27-hour execution plan knows where to spend time:

| Priority | Resource | Stage | Effort | Status |
|:---:|---|---|---|---|
| **P0** | **Open-Meteo** (forecast + archive) | SENSE / PREDICT | < 1 h | Free, no auth, drop-in |
| **P0** | **OSMnx + Overpass** | Data prep | 1–2 h | Free, no auth |
| **P0** | **Google OR-Tools** | OPTIMIZE | < 1 h | `pip install ortools` |
| **P0** | **H3-py** | ASSESS | < 30 min | `pip install h3` |
| **P0** | **GeoPandas** | Throughout | < 30 min | Standard GIS stack |
| **P1** | **Open-Meteo Elevation** | Data prep / ASSESS | 30 min | Batch fetch at build |
| **P1** | **Geofabrik OSM Extract** | Data prep | 30 min | Direct download |
| **P1** | **SACHET CAP RSS** | SENSE | 1 h | Public RSS, parse XML |
| **P1** | **India Flood Inventory** | Data prep | 1–2 h | Download + spatial join |
| **P1** | **MapLibre GL JS + deck.gl** | Frontend | 2–4 h | npm install |
| **P2** | **OpenTopography DEM** | Data prep | 1 h | Free account needed |
| **P2** | **data.gov.in** | Data prep | 1–2 h | Free registration |
| **P2** | **PyLandslide / iamtekson LSM** | PREDICT | 2 h (study only) | Reference only |
| **P2** | **gboeing/travel-time-prediction** | PREDICT | 1 h (study only) | Reference only |
| **P3** | **VROOM** | OPTIMIZE | 2 h | Docker; warm-start only |
| **P3** | **IMD API** | SENSE | 2+ h | Registration + wait |
| **P3** | **ISRO Bhuvan** | Data prep | 2+ h | Registration + wait |
| **FUTURE** | **OpenTripPlanner** | OPTIMIZE | N/A | Multimodal scope only |

> [!IMPORTANT]
> P0 resources cost zero auth overhead and zero wait time. They should be integrated in the first
> 4 hours of the build. P1 resources require a simple download or public feed — integrate in hours
> 4–10. P2/P3 resources depend on registration or are reference-only — schedule only if P0/P1
> integration is complete and time remains.
